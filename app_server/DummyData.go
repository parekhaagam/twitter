...
/*
	...
	fmt.Println("tid", tid)
	tid = StorageInsertTweets(globals.User{"manish.n"}, "SDE at Google")
	fmt.Println("tid", tid)
	tid = StorageInsertTweets(globals.User{"chandler"}, "Could I be wearing anymore clothes")
	fmt.Println("tid", tid)
	tid = StorageInsertTweets(globals.User{"srk"}, "Zero releasing December 2018")
	fmt.Println("tid", tid)
